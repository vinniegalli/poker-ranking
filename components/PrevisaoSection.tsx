...
export function PrevisaoSection({ ranking, caixaTotal, year }: Props) {
  ...
  const qualifiers =
    previsao && totalSessions > 0
      ? ranking
          .filter(
            (p) => (p.participacoes / totalSessions) * 100 >= previsao.min_freq,
          )
  ...
  return (
    <div className="space-y-3">
          ...
          {loading ? (
            ...
            <div className="p-5 space-y-4">
              ...
              {totalSessions === 0 ? (
                ...
                <p className="text-sm text-muted-foreground text-center py-2">
                  Nenhum jogador com ≥{previsao.min_freq}% de frequência em{" "}
                ...
                <div className="space-y-2">
                  {qualifiers.map((p, i) => {
                    ...
                    return (
                      <div
                        key={p.player_id}
                        className="flex items-center gap-3 bg-secondary/30 rounded-lg px-3 py-3"
                      >
                        <span className="text-lg w-8 text-center flex-shrink-0 leading-none">
                          {MEDALS[i]}
                        </span>
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">
                            {freq}% frequência · {p.participacoes}/
                            {totalSessions} sessões
                          </p>
                        </div>
                        <div className="text-right flex-shrink-0">
                          <p className="font-mono-numbers font-bold text-gold">
                            {formatBRL(prize)}
                          </p>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent
          className="sm:max-w-sm card-border bg-card"
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 bg-secondary rounded-lg px-4 py-2.5">
                <button
                  onClick={() =>
                    handlePlacementsChange(Math.max(1, editPlacements - 1))
                  }
                  className="text-foreground/60 hover:text-foreground font-bold text-xl w-6 leading-none"
                >
                  −
                ...
                <button
                  onClick={() =>
                    handlePlacementsChange(Math.min(5, editPlacements + 1))
                  }
                  className="text-gold hover:text-gold/70 font-bold text-xl w-6 leading-none"
                >
                  +
